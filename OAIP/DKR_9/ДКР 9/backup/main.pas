...
begin
  ...
  if fEdit.ModalResult = mrOk then begin
    ...
    SG.Cells[6, SG.Row]:= fEdit.eDlit.Text;
  end;
end;
...
begin
  ...
  SG.Cells[2, 0]:= 'Жанр';
  SG.Cells[3, 0]:= 'Год';
  SG.Cells[4, 0]:= 'Смотрел';
  SG.Cells[5, 0]:= 'Режиссер';
  SG.Cells[6, 0]:= 'Длительность (мин)';
  SG.ColWidths[0]:= 150;
  SG.ColWidths[1]:= 150;
  SG.ColWidths[2]:= 150;
  SG.ColWidths[3]:= 150;
  ...
  try
    ...
    while not Eof(f) do begin
      ...
      //добавляем в сетку новую строку, и заполняем её:
        ...
        SG.Cells[6, SG.RowCount-1]:= MyCont.Duration;
    end;
  finally
    CloseFile(f);
  end;
