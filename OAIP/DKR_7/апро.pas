...
function KU( i: integer): integer; // параметризация
begin
  if i > 0 then //база
  begin
    KR(i - 1);
    LineUP(x0, y0, step); //декомпозиция
    KU(i - 1);
...
begin
  case key of
    ...
    VK_q : begin step:=step+2; per; end; //увеличение
    VK_w : begin if step > 0 then step:=step-2; per; end; //уменьшение
    VK_z : begin if iter<10 then iter:=iter+1; per; end; //итерация +1
    VK_x : begin if iter>0 then iter:=iter-1; per; end; //итерация -1
  end;
end;
